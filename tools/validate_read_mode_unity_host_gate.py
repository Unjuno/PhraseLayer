...
def forbid(text: str, fragment: str, label: str) -> None:
    if fragment in text:
        raise GateError(f"{label} contains forbidden device dependency: {fragment}")
...
def validate() -> dict[str, object]:
    ...
    for fragment in (
        ...
        'assert build["architecture"] == "ARM64"',
        'assert build["scripting_backend"] == "IL2CPP"',
        ...
        'assert visual["mask_shader_reasserted"] is True',
        ...
        require(workflow, fragment, "Read Mode Unity host workflow")

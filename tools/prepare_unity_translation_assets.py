...
This command never downloads a model. It accepts the metadata report produced by
`tools/probe_translation_export.py` plus the exact local export directory that report describes.
Every file is size/SHA-256 verified before and after copying. A report that did not pass token-exact
reference parity is rejected, and no claim of Unity compatibility is made by staging alone.
"""
...
def load_and_validate_report(report_path: Path) -> dict[str, Any]:
    ...
    if report.get("model_id") != EXPECTED_MODEL_ID:
        raise PrepareTranslationError(
            f"translation probe model_id must be {EXPECTED_MODEL_ID!r}; found {report.get('model_id')!r}"
        )
    if report.get("revision") != EXPECTED_REVISION:
        raise PrepareTranslationError(
            f"translation probe revision must be {EXPECTED_REVISION}; found {report.get('revision')!r}"
        )
    ...
    if report.get("runtime_status") != EXPECTED_RUNTIME_STATUS:
        raise PrepareTranslationError(
            "translation probe must remain unverified for real Unity import while staging local assets"
        )
    ...
    for item in files:
        ...
        if relative.suffix.lower() == ".onnx":
            ...
            if not isinstance(inspection, dict):
                raise PrepareTranslationError(f"ONNX artifact lacks graph inspection: {canonical}")
    ...
    if onnx_count < 2:
        raise PrepareTranslationError(
            f"translation probe must describe at least encoder and decoder ONNX artifacts; found {onnx_count}"
        )
    return report
...
def verify_source(export_root: Path, relative: Path, expected_size: int, expected_sha: str) -> Path:
    ...
    if size != expected_size or digest != expected_sha:
        raise PrepareTranslationError(
            f"translation artifact identity mismatch for {relative.as_posix()}: "
            f"expected {expected_size}/{expected_sha}, got {size}/{digest}"
        )
    return source

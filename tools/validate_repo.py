...
for model in manifest.get("candidates", []):
    ...
    if not str(model.get("purpose", "")).startswith("ocr-"):
        continue
    for key in ("revision", "artifact", "artifact_size_bytes", "artifact_sha256", "format", "runtime_target",
                "runtime_compatibility", "quantization", "source_precision"):
    ...
    require(model.get("runtime_target") == "com.unity.ai.inference@2.2.1", f"OCR runtime target drift: {model_id}")
    ...
    for support in support_artifacts:
        ...
        require(isinstance(support_path, str) and bool(support_path), f"OCR support artifact path missing: {model_id}")
        if isinstance(support_path, str) and support_path:
            ...
            require(support_path not in seen_support_paths, f"OCR duplicate support artifact path: {model_id}:{support_path}")
...
required_files = [
    TOOLS / "stage_models.py", TOOLS / "extract_ppocr_dictionary.py", TOOLS / "test_extract_ppocr_dictionary.py",
    TOOLS / "validate_ppocr_dictionary_manifest_wiring.py",
    CORE / "PaddleOcrRuntimeContract.cs", CORE / "PaddleOcrDictionaryManifest.cs", CORE / "PaddleCtcPackedOutput.cs",
    UNITY / "ProjectSettings" / "ProjectVersion.txt", UNITY / "Packages" / "manifest.json",
...
validate_markers(TOOLS / "stage_models.py", "model stager",
                 ("support_artifacts", "--include-support", "artifact_kind", "PhraseLayer-model-stager/2"))
validate_markers(TOOLS / "extract_ppocr_dictionary.py", "PP-OCR dictionary extractor",
                 ("paddle-inference-yaml", "character_dict", "decode_yaml_scalar", "generated_artifact_sha256", "raw token count mismatch", "another space token"))
validate_markers(CORE / "PaddleOcrRuntimeContract.cs", "Core PP-OCR runtime contract",
                 ("PaddleDbProbabilityMap.FromTensor(outputShape, outputValues)", "Recognizer output must be [1,time,class]",
...
validate_markers(UNITY / "Assets" / "Scripts" / "PaddleDetectorRawOutputExtensions.cs", "Unity PP-OCR DB decoder",
...
validate_markers(UNITY / "Assets" / "Scripts" / "UnityPaddleOcrRecognizerRuntime.cs", "Unity PP-OCR recognizer runtime",
                 ("PaddleOcrV6TinyRecognitionPreprocess.CreateResizeTransform", "PaddleCtcGreedyDecoder.DecodeFromPredictions",
                  "reducedOutputWorker.Schedule(inputTensor)",
                  "using (var parityWorker = new Worker(ModelLoader.Load(modelAsset), backendType))", "parityWorker.Schedule(inputTensor)",
                  "Functional.ArgMax(probabilities, dim: -1, keepdim: false)", "Functional.ReduceMax(probabilities, dim: -1, keepdim: false)",
                  "UsesGpuCtcReduction => backendType == BackendType.GPUCompute", "RetainsFullOutputWorker => false", "ReadbackAndClone()"))
validate_markers(UNITY / "Assets" / "Scripts" / "UnityPaddleOcrCropRectifier.cs", "Unity PP-OCR crop rectifier",
...
validate_markers(UNITY / "Assets" / "Scripts" / "UnityPaddleOcrEngine.cs", "Unity PP-OCR end-to-end engine",
                 ("detector.Execute(texture, frame.Width, frame.Height)", "PaddleOcrRuntimeContract.ValidateDetector",
                  "DecodeV6TinyQuads(dbSpec)", "PaddleOcrReadingOrder.Sort", "cropRectifier.Rectify(texture, detection.ImageBounds)",
                  "recognizer.ExecuteReduced(", "PaddleOcrRuntimeContract.ValidateRecognizerReduced(", "recognizerOutput.Decode(characterDictionary)",
                  "RuntimeContractReport", "PaddleOcrObservationAssembler.Assemble", "ValidateRecognizerConfidence"))
validate_markers(UNITY / "Assets" / "Scripts" / "UnityPaddleOcrDictionaryManifest.cs", "Unity PP-OCR dictionary manifest",
...
validate_markers(UNITY / "Assets" / "Scripts" / "UnityPaddleOcrBootstrapBehaviour.cs", "Unity PP-OCR scene bootstrap",
                 ("characterDictionaryManifest", "UnityPaddleOcrDictionaryManifest.Validate(",
                  "PaddleOcrCharacterDictionary.Parse(characterDictionary.text, useSpaceCharacter)", "new UnityPaddleOcrEngine(",
                  "runtimeDriver.ConfigureEngine(created)", "DictionaryManifestReport", "RuntimeContractReport"))
validate_markers(UNITY / "Assets" / "Resources" / "PaddleOcrPerspectiveCrop.shader", "Unity PP-OCR crop shader",
                 ("_RotateCCW90", "preRotationTop = float2(1.0 - outputTop.y, outputTop.x)", "sourceUv = saturate(sourceUv)", "tex2D(_MainTex, sourceUv)"))
validate_markers(UNITY / "Assets" / "Resources" / "PaddleOcrRecognizerPreprocess.shader", "Unity PP-OCR recognizer preprocess shader",
...
expected_define = {"name": "com.unity.ai.inference", "expression": "[2.2.1,2.3.0)", "define": "PHRASELAYER_UNITY_AI_INFERENCE_2_2"}
require(expected_define in unity_asmdef.get("versionDefines", []), "PhraseLayer.Unity.asmdef must gate the reviewed Unity Inference 2.2.x API surface")
unity_manifest = json.loads((UNITY / "Packages" / "manifest.json").read_text(encoding="utf-8"))
deps = unity_manifest.get("dependencies", {})
for package, expected in {
    "com.unjuno.phraselayer.core": "file:../../src/PhraseLayer.Core", "com.meta.xr.mrutilitykit": "85.0.0",
    "com.unity.ai.inference": "2.2.1", "com.unity.xr.management": "4.5.4", "com.unity.xr.meta-openxr": "2.2.1",
    "com.unity.xr.openxr": "1.15.1", "com.unity.ugui": "2.0.0",
}.items():
    require(deps.get(package) == expected, f"Unity package drift: {package} expected {expected} but found {deps.get(package)}")
...
if violations:
    raise SystemExit("\n".join(violations))
print(f"PASS: {len(list(CORE.rglob('*.cs')))} core files; Core/Unity boundaries, fully pinned OCR artifacts, "

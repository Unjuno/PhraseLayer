...
#if UNITY_5_3_OR_NEWER
    /// <summary>
    /// Local profile store under Application.persistentDataPath.
    /// Write a complete temp, move an existing primary to backup, promote temp, then remove backup.
    /// A backup-only recovery state must retain its sole valid copy until the new primary exists.
    /// Load recovers a missing primary from backup. This does not establish fsync/power-loss durability.
    ...
    public sealed class UnityLearnerProfileStore : ILearnerProfileStore
    {
        ...
        public UnityLearnerProfileStore(string filePath = null)
        {
            var resolved = string.IsNullOrWhiteSpace(filePath)
                ? Path.Combine(Application.persistentDataPath, "PhraseLayer", DefaultFileName) : filePath;
        ...
        {
            if (File.Exists(FilePath)) return ReadSnapshot(FilePath);
            if (File.Exists(BackupPath)) return ReadSnapshot(BackupPath);
            return null;
        }
        public void Save(LearnerProfileSnapshot snapshot)
        {
            ...
            if (string.IsNullOrEmpty(directory)) throw new InvalidOperationException("Learner profile path must have a parent directory.");
            ...
            // Deleting an old backup is safe only while a primary still protects the previous state.
            // With no primary, keep the recovery backup until temp promotion succeeds.
        ...
        private static LearnerProfileSnapshot ReadSnapshot(string path)
        {
            ...
            if (dto == null) throw new InvalidDataException("Learner profile JSON did not deserialize to an object: " + path);
        ...
        private sealed class ProfileDto
        {
            ...
            public static ProfileDto FromSnapshot(LearnerProfileSnapshot snapshot)
            {
                ...
                foreach (var entry in snapshot.Entries) dto.entries.Add(new EntryDto { text = entry.Text, understanding = entry.Understanding });
                return dto;
            }
            public LearnerProfileSnapshot ToSnapshot()
            {
                if (entries == null) throw new InvalidDataException("Learner profile entries array is missing.");
                var converted = new List<LearnerKnowledgeEntry>(entries.Count);
                foreach (var entry in entries)
                {
                    if (entry == null) throw new InvalidDataException("Learner profile contains a null entry.");
...
#else
    /// <summary>Host-CI fallback; real persistence requires a Unity runtime.</summary>
    public sealed class UnityLearnerProfileStore : ILearnerProfileStore
    {
        public LearnerProfileSnapshot Load() { throw new NotSupportedException("UnityLearnerProfileStore requires a Unity runtime."); }
        public void Save(LearnerProfileSnapshot snapshot)
        {
            if (snapshot == null) throw new ArgumentNullException(nameof(snapshot));
